...
class AppControlDatabaseMigrationTest {
    ...
    @Test
    fun migration2To3ShouldCreateDiagnosticsEventsTableWithoutDroppingTaskRuns() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val databaseName = "migration-2-3-test.db"
        context.deleteDatabase(databaseName)

        createVersion2Database(context, databaseName).close()
        ...
        val migratedHelper = createOpenHelper(
            ...
            callback = object : SupportSQLiteOpenHelper.Callback(3) {
                ...
                override fun onUpgrade(db: SupportSQLiteDatabase, oldVersion: Int, newVersion: Int) {
                    AppControlDatabase.MIGRATION_2_3.migrate(db)
                }
            },
        )

        migratedHelper.writableDatabase.use { database ->
            database.query("PRAGMA table_info(diagnostic_events)").use { cursor ->
                var foundPayloadColumn = false
                while (cursor.moveToNext()) {
                    if (cursor.getString(1) == "payloadJson") {
                        foundPayloadColumn = true
                        assertEquals("TEXT", cursor.getString(2))
                        assertEquals(1, cursor.getInt(3))
                    }
                }
                assertEquals(true, foundPayloadColumn)
            }
    ...
    private fun createVersion2Database(context: Context, databaseName: String): SupportSQLiteOpenHelper {
        val helper = createOpenHelper(
            context = context,
            name = databaseName,
            callback = object : SupportSQLiteOpenHelper.Callback(2) {
